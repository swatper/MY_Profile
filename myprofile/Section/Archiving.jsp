<%@ page language="java" contentType="application/json; charset=UTF-8" pageEncoding="UTF-8"%>
<div class="Archiving_container">
    <div class="Archiving_title">Archiving</div>

    <div class="Archiving_list">
        <!--GitHub 내용-->
        <a class="GitHub" href="https://github.com/swatper">
            <img src="IMG/Git_Logo.png">
            <div class="Info">프로젝트 및 알고리즘 공부 소스코드 저장소</div>
        </a>
        <!--Notion 내용-->
        <a class="Notion" href="https://www.notion.so/115d6c7cb36580aa80b1f57a2152f9e0?pvs=4">
            <img src="IMG/Notion_Logo.png">
            <div class="Info">학교 및 개인 공부 내용 기록 저장소</div>
        </a>
        <!--SolvedAc 내용-->
        <a class="SolvedAc" href="https://solved.ac/sh010510">
            <img src="https://mazassumnida.wtf/api/v2/generate_badge?boj=sh010510">
        </a>
        <!--GitHub언어 내용-->
        <a class="GitRate" href="https://github.com/swatper" >
            <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=swatper&layout=compact&theme=tokyonight">
        </a>
    </div>
</div>